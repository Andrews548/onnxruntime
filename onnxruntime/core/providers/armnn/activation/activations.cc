...
template <typename T>
thread_local std::map<OpKernel*, ARMNNRelu> Relu<T>::reluLayers;

template <typename T>
thread_local armnn::IRuntimePtr Relu<T>::run = Relu<T>::initRuntime();
...
Status Relu<T>::Compute(OpKernelContext* context) const {
  ...
  if (it == Relu::reluLayers.end()) {
    ...
    trelu.networkIdentifier = std::make_shared<armnn::NetworkId>(networkId);
    ...
    Relu::run->LoadNetwork(*(trelu.networkIdentifier.get()), std::move(optNet));


    std::pair<ReluLayersIterator, bool> ret;
    ret = Relu::reluLayers.insert(std::pair<OpKernel*, ARMNNRelu>((OpKernel*)this, trelu));
    pRelu = &ret.first->second;

  } else {
    pRelu = &it->second;
  }

  armnn::InputTensors inputTensors{{0, armnn::ConstTensor(Relu::run->GetInputTensorInfo(*(pRelu->networkIdentifier.get()), 0),
                                                          src_data)}};
  armnn::OutputTensors outputTensors{{0, armnn::Tensor(Relu::run->GetOutputTensorInfo(*(pRelu->networkIdentifier.get()), 0),
                                                       dst_data)}};

  Relu::run->EnqueueWorkload(*(pRelu->networkIdentifier.get()), inputTensors, outputTensors);
